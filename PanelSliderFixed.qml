...
Item {
  ...
  MouseArea {
    ...
    // Map the pointer across the knob's usable travel (track minus one knob
    // width), offset by half a knob. At the knob's clamped extremes this
    // yields minimum/maximum exactly, and the midpoint still maps to the
    // value midpoint (no jump when grabbing the knob).
    function valueFromX(x) {
      var usable = Math.max(1, track.width - root.knobSize)
      var centered = x - root.knobSize / 2
      var clamped = Math.max(0, Math.min(usable, centered))
      var raw = root.minimum + (clamped / usable) * root.range
